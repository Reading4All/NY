...
//const template = inputs => htl.html`<div>${
//  (Array.isArray(inputs) ? inputs : Object.values(inputs)).map(input => htl.html`<span style="display:inline-block;margin:4px;">${input}</span>`)
//}${footer}`;

const template = inputs => htl.html`<div style="max-width:100%;white-space:normal;">${
  (Array.isArray(inputs) ? inputs : Object.values(inputs)).map(input => htl.html`<span style="display:inline-block;margin:4px;">${input}</span>`)
}${footer}`;
