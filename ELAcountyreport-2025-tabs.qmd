---
#title: "NY Reading Proficiency - County Report"
format: 
  dashboard: 
    orientation: columns
    expandable: true
    scrolling: false
#    logo: "images/OSD-favicon.png"
#    nav-buttons:
#        - icon: github
#          href: https://github.com/Reading4All/NY
---


```{r}
#| echo: false
#| include: false
library(tidyverse)
library(tidyr)


###### {r, read csv, format df}
YEAR <-  2025 #YEAR ENDING SPRING DATA, so CSV with same year as selected

###### read csv data
## locally:
csv_file <- paste0("C:\\Users\\srene\\Documents\\ADVOCACY\\My_work\\NYSED-REPORTS\\SOURCE_DATA\\NYSED_DBs\\CLEANED_Data\\SOURCEDATA_ELA-SPRING-", YEAR, ".csv")
## or from Github:
## csv_file <- paste0("https://raw.githubusercontent.com/Reading4All/NY/refs/heads/main/data/SOURCEDATA_ELA-SPRING-", YEAR, ".csv")

shared_data <- read.csv(csv_file)

###### {r, Filter to keep all data we made need regardless of which year is selected}

## Only Keep District Data
shared_data<-shared_data[(shared_data$ENTITY_TYPE == "DISTRICT"),]
## Only Keep Spring (year-end) Data
shared_data<-shared_data[(shared_data$SEASON == "Spring"),]

# Calculate and create new col 'Prof_Gap' for SWD and Econ.Dis rows (%prof SWD or EconDis - %prof GenEd or NotEconDis)
shared_data <- shared_data %>%
  left_join(
    shared_data %>%
      filter(SUBGROUP_NAME == "General Education Students") %>%
      select(ENTITY_NAME, ASSESSMENT_NAME, Gen_Ed_Pct = Prof_Pct),
    by = c("ENTITY_NAME", "ASSESSMENT_NAME")
  ) %>%
  left_join(
    shared_data %>%
      filter(SUBGROUP_NAME == "Not Economically Disadvantaged") %>%
      select(ENTITY_NAME, ASSESSMENT_NAME, Not_EconDis_Pct = Prof_Pct),
    by = c("ENTITY_NAME", "ASSESSMENT_NAME")
  ) %>%
  mutate(
    Prof_Gap = case_when(
      SUBGROUP_NAME == "Students with Disabilities"    ~ Gen_Ed_Pct - Prof_Pct,
      SUBGROUP_NAME == "Economically Disadvantaged"    ~ Not_EconDis_Pct - Prof_Pct,
      TRUE                                             ~ NA_real_
    )
  ) %>%
  select(-Gen_Ed_Pct, -Not_EconDis_Pct)  # drop helper columns

# Get rid of NA Prof rates, and sort by Prof rate
shared_data <- shared_data %>% 
  filter(!is.na(Prof_Pct)) %>% 
  dplyr::arrange(Prof_Pct, NotProf_Pct)
# Reindex the df, so the plot order will stay as we want:
if (nrow(shared_data) > 0){
rownames(shared_data) <- 1:nrow(shared_data)
}

######################################################

# df to ojs
ojs_define(df_to_js = shared_data) 

```

```{ojs}
//| output: false

// Get the data from R and make it usable (transpose it) in JS 
shared_data = transpose(df_to_js)

// guard function
// makes a submit and reset button to use after multiple inputs made, see https://observablehq.com/@mootari/inputs-submit

function guard(fn, options = {}) {
  const {
    submitLabel = "Submit",
    resetLabel = "Reset",
    required = false,
    resubmit = true,
    width = "fit-content", 
//    width = "min-content",
    justify = "start",
    valid = input => !input.querySelector(":invalid"),
  } = options;

  const onSubmit = () => {
    value = input.value;
    submit.disabled = !resubmit || invalid;
    reset.disabled = true;
    wrapper.dispatchEvent(new Event("input", { bubbles: true }));
  };
  const onReset = () => {
    input.value = value;
    submit.disabled = !resubmit || invalid;
    reset.disabled = true;
  };
  const onInnerInputCapture = () => {
    invalid = true;
    submit.disabled = true;
  };
  const onInnerInput = e => {
    e.stopPropagation();
    submit.disabled = false;
    reset.disabled = false;
  };
  
  const submit = htl.html`<button ${{disabled: !resubmit && !required, onclick: onSubmit}}>${submitLabel}`;
  const reset = htl.html`<button ${{disabled: true, onclick: onReset}}>${resetLabel}`;
  const footer = htl.html`<div><hr style="padding:0;margin:10px 0"><div style="display:flex;gap:1ch;justify-content:${justify}">${submit} ${reset}`;

  const template = inputs => htl.html`<div>${
    Array.isArray(inputs) ? inputs : Object.values(inputs)
  }${footer}`;
  
  const input = fn({submit, reset, footer, template, onSubmit, onReset});
  let invalid = !!input.querySelector(":invalid");
  submit.disabled = !resubmit || invalid;

  input.addEventListener("input", onInnerInputCapture, {capture: true});
  input.addEventListener("input", onInnerInput);
  let value = required ? undefined : input.value;
  const wrapper = htl.html`<div style="width:${width}">${input}`;
  //const wrapper = htl.html`<div>${input}`;
  wrapper.addEventListener("submit", onSubmit);
  return Object.defineProperty(wrapper, "value", {
    get: () => value,
    set: (v) => { input.value = v },
  });
}

// save unique Counties so I can use these in select popup
uniqueCounties = new Set(shared_data.map(d => d.COUNTY_DESC));
```
## User Inputs {.sidebar}
```{ojs}

viewof inputSels = guard(
  ({ template }) => Inputs.form(
    {
      county: Inputs.text({ 
        value: "NASSAU",
        label: "County:",
 //       placeholder: "Enter County…",
        datalist: uniqueCounties,
        unique: true,
        validate: v => shared_data.map(d => d.COUNTY_DESC).includes(v.value.toUpperCase()),
        sort: true,
        required: true,
        width: 75
        }),
      grade: Inputs.select(new Map([ ["Grade 3", "ELA3"], ["Grade 4", "ELA4"], ["Grade 5", "ELA5"], ["Grade 6", "ELA6"], ["Grade 7", "ELA7"], ["Grade 8", "ELA8"], ["All Grades (3-8)", "ELA3_8"] ]), {
        value: "ELA3", 
        label: "Grade:",
        sort: true,
        required: true,
        width: 75
      }),
      
      cohort: Inputs.select(new Map([ ["IEP / no IEP", "SWD"], ["Economically Disadvantaged / not", "Econ"] ]), {
        value: "SWD",   // ← was "SWD", which doesn't exist in the map
        label: "Cohorts to compare:",
        required: true,
        width: 75
      })
    },
    { template }
  ),
  // Options. Override the label of the reset button, require inputs pass validation
  { resetLabel: "Reset", required: true }
);
```

```{ojs}
//| output: false
_autoSubmit = {
  setTimeout(() => {
    const submitBtn = Array.from(document.querySelectorAll("button"))
      .find(b => b.textContent.trim() === "Submit");
    if (submitBtn) submitBtn.click();
  }, 1000);
}
```

```{ojs}
//| output: false
cohortPlot1 = inputSels.cohort == "SWD" ? "General Education Students" : "Not Economically Disadvantaged"
cohortPlot1_short = inputSels.cohort == "SWD" ? "no IEP (Gen Ed)" : "Not EconDis."

```

```{ojs}
//| output: false
cohortPlot2 = inputSels.cohort == "SWD" ? "Students with Disabilities" : "Economically Disadvantaged"
cohortPlot2_short = inputSels.cohort == "SWD" ? "IEP (SWD)" : "EconDis."
```

```{ojs}
//| output: false

// Plot3 (Gap) uses SWD or Econ Dis rows (which have calculated prof_gap col values)
cohortPlot3 = inputSels.cohort == "SWD" ? "Students with Disabilities" : "Economically Disadvantaged"
```

```{ojs}
//| output: false
shared_dataFiltered1 = shared_data.filter(d =>
  inputSels.county.toUpperCase() == d.COUNTY_DESC &&
  inputSels.grade  == d.ASSESSMENT_NAME &&
  cohortPlot1      == d.SUBGROUP_NAME
)
```

```{ojs}
//| output: false
shared_dataFiltered2 = shared_data.filter(d =>
  inputSels.county.toUpperCase() == d.COUNTY_DESC &&
  inputSels.grade  == d.ASSESSMENT_NAME &&
  cohortPlot2      == d.SUBGROUP_NAME
)
```

```{ojs}
//| output: false
shared_dataFiltered3 = shared_data.filter(d =>
  inputSels.county.toUpperCase() == d.COUNTY_DESC &&
  inputSels.grade  == d.ASSESSMENT_NAME &&
  cohortPlot3      == d.SUBGROUP_NAME
)
```
## Plots
::: {.panel-tabset}
### ${cohortPlot2_short}
```{ojs}

fig2 = {
  const width = 1000;
  const barHeight = 24;
  const height = 150 + shared_dataFiltered2.length * barHeight;

  // Build stacked segments for each row
  const proficient = shared_dataFiltered2.map(d => ({
    entity: d.ENTITY_NAME,
    x1: 0,
    x2: d.Prof_Pct,
    category: "Proficient",
    label: String(d.Prof_Count),
    color: "rgba(3,129,179,1)"
  }));

  const notProficient = shared_dataFiltered2.map(d => ({
    entity: d.ENTITY_NAME,
    x1: d.Prof_Pct,
    x2: d.Prof_Pct + d.NotProf_Pct,
    category: "Not Proficient",
    label: String(d.NotProf_Count),
    color: "rgba(170,74,68,1)"
  }));

  const notTested = shared_dataFiltered2.map(d => ({
    entity: d.ENTITY_NAME,
    x1: d.Prof_Pct + d.NotProf_Pct,
    x2: d.Prof_Pct + d.NotProf_Pct + d.NotTested_Pct,
    category: "Not Tested",
    label: String(d.NOT_TESTED),
    color: "rgba(209,209,209,1)"
  }));

  const segments = [...proficient, ...notProficient, ...notTested];

  const entityOrder = shared_dataFiltered2.map(d => d.ENTITY_NAME).reverse();

  return Plot.plot({
    width,
    height,
    marginLeft: 175,   // adjust to fit longest entity name
    marginRight: 60,
    marginTop: 15,
    marginBottom: 40,
    
    style: {
      fontSize: "14px" // Changes all plot text sizes
    },
    
    title: cohortPlot2,
    subtitle: `County: ${inputSels.county}, Grade: ${inputSels.grade}`,
    caption: html`Data Source (downloadable): <a href="https://data.nysed.gov/downloads.php">NYSED Report Card - downloadable</a><br>
              Data Source (website): <a href="https://data.nysed.gov/">NYSED Report Card - website</a>`,

    x: {
      label: "% of Students",
      labelAnchor: "center",
      labelArrow: "none",
      domain: [0, 100]
    },
    y: {
      label: null,
      domain: entityOrder,   // preserves original row order
      tickSize: 0
    },
    color: {
      domain: ["Proficient", "Not Proficient", "Not Tested"],
      range: ["rgba(3,129,179,1)", "rgba(170,74,68,1)", "rgba(209,209,209,1)"],
      legend: true
    },

    marks: [
      // Stacked bars drawn as rects
      Plot.barX(segments, {
        x1: d => d.x1,
        x2: d => d.x2,
        y: d => d.entity,
        fill: d => d.color,
        stroke: "black",
        strokeWidth: 0.5
      }),

      // Count labels inside each bar segment
      Plot.text(segments, {
//        x: d => d.x1 + (d.x2 - d.x1) / 2,  // center of each segment
        x: d => d.x1 + 1,  // start of each segment
        y: d => d.entity,
        text: d => d.x2 - d.x1 > 2 ? d.label : "",  // hide label if segment too narrow
        fill: "black",
        textAnchor: "start"
      }),

      // Total count label to the right of each bar
      Plot.text(shared_dataFiltered2, {
        x: 100,
        y: d => d.ENTITY_NAME,
        text: d => `(${d.TOTAL_COUNT})`,
        fill: "black",
        textAnchor: "start",
        dx: 5
      }),

      // Vertical reference line at 95%
      Plot.ruleX([95], {
        stroke: "black",
        strokeWidth: 1,
        strokeDasharray: "4,3"
      }),

      // Annotation label at top of the 95% line
      Plot.text([">95% CAN!    "], {
        frameAnchor: "top-right", 
        dy: -12 // Adjust vertical offset to move it slightly
    }),
    ]
  });
}
```
### ${cohortPlot1_short}
```{ojs}

fig1 = {
  const width = 1000;
  const barHeight = 24;
  const height = 150 + shared_dataFiltered1.length * barHeight;

  // Build stacked segments for each row
  const proficient = shared_dataFiltered1.map(d => ({
    entity: d.ENTITY_NAME,
    x1: 0,
    x2: d.Prof_Pct,
    category: "Proficient",
    label: String(d.Prof_Count),
    color: "rgba(3,129,179,1)"
  }));

  const notProficient = shared_dataFiltered1.map(d => ({
    entity: d.ENTITY_NAME,
    x1: d.Prof_Pct,
    x2: d.Prof_Pct + d.NotProf_Pct,
    category: "Not Proficient",
    label: String(d.NotProf_Count),
    color: "rgba(170,74,68,1)"
  }));

  const notTested = shared_dataFiltered1.map(d => ({
    entity: d.ENTITY_NAME,
    x1: d.Prof_Pct + d.NotProf_Pct,
    x2: d.Prof_Pct + d.NotProf_Pct + d.NotTested_Pct,
    category: "Not Tested",
    label: String(d.NOT_TESTED),
    color: "rgba(209,209,209,1)"
  }));

  const segments = [...proficient, ...notProficient, ...notTested];

  const entityOrder = shared_dataFiltered1.map(d => d.ENTITY_NAME).reverse();

  return Plot.plot({
    width,
    height,
    marginLeft: 175,   // adjust to fit longest entity name
    marginRight: 60,
    marginTop: 15,
    marginBottom: 60,
    
    style: {
      fontSize: "14px" // Changes all plot text sizes
    },
    
    title: cohortPlot1,
    subtitle: `County: ${inputSels.county}, Grade: ${inputSels.grade}`,
    caption: html`Data Source (downloadable): <a href="https://data.nysed.gov/downloads.php">NYSED Report Card - downloadable</a><br>
              Data Source (website): <a href="https://data.nysed.gov/">NYSED Report Card - website</a>`,
              
    x: {
      label: "% of Students",
      labelAnchor: "center",
      labelArrow: "none",
      domain: [0, 100]
    },
    y: {
      label: null,
      domain: entityOrder,   // preserves original row order
      tickSize: 0
    },
    color: {
      domain: ["Proficient", "Not Proficient", "Not Tested"],
      range: ["rgba(3,129,179,1)", "rgba(170,74,68,1)", "rgba(209,209,209,1)"],
      legend: true
    },

    marks: [
      // Stacked bars drawn as rects
      Plot.barX(segments, {
        x1: d => d.x1,
        x2: d => d.x2,
        y: d => d.entity,
        fill: d => d.color,
        stroke: "black",
        strokeWidth: 0.5
      }),

      // Count labels inside each bar segment
      Plot.text(segments, {
//        x: d => d.x1 + (d.x2 - d.x1) / 2,  // center of each segment
        x: d => d.x1 + 1,  // start of each segment
        y: d => d.entity,
        text: d => d.x2 - d.x1 > 2 ? d.label : "",  // hide label if segment too narrow
        fill: "black",
        textAnchor: "start"
      }),

      // Total count label to the right of each bar
      Plot.text(shared_dataFiltered1, {
        x: 100,
        y: d => d.ENTITY_NAME,
        text: d => `(${d.TOTAL_COUNT})`,
        fill: "black",
        textAnchor: "start",
        dx: 5
      }),

      // Vertical reference line at 95%
      Plot.ruleX([95], {
        stroke: "black",
        strokeWidth: 1,
        strokeDasharray: "4,3"
      }),

      // Annotation label at top of the 95% line
      Plot.text([">95% CAN!    "], {
        frameAnchor: "top-right", 
        dy: -12 // Adjust vertical offset to move it slightly
    }),
    ]
  });
}
```
### GAP
```{ojs}
fig3 = {
  const width = 1000;
  const barHeight = 24;
  const height = 150 + shared_dataFiltered3.length * barHeight;

  // Build stacked segments for each row
  const proficient = shared_dataFiltered3.map(d => ({
    entity: d.ENTITY_NAME,
    x1: 0,
    x2: d.Prof_Gap,
    category: "Proficiency Gap",
    color: "rgba(3,129,179,1)"
  }));

  const segments = [...proficient];

  const entityOrder = shared_dataFiltered3.map(d => d.ENTITY_NAME).reverse();

  return Plot.plot({
    width,
    height,
    marginLeft: 175,   // adjust to fit longest entity name
    marginRight: 60,
    marginTop: 15,
    marginBottom: 60,
    
    style: {
      fontSize: "14px" // Changes all plot text sizes
    },
    
    title: `Equity GAP: ${cohortPlot1} vs. ${cohortPlot2}`,
    subtitle: `County: ${inputSels.county}, Grade: ${inputSels.grade}`,
    caption: html`Gap = %Proficient(SWD or Econ.Dis) - %Proficient(Not SWD or Not Econ.Dis)<br>
          Data Source (downloadable): <a href="https://data.nysed.gov/downloads.php">NYSED Report Card - downloadable</a>`,
              
    x: {
      label: "% Profiency Gap",
      labelAnchor: "center",
      labelArrow: "none",
      domain: [0, 100]
    },
    y: {
      label: null,
      domain: entityOrder,   // preserves original row order
      tickSize: 0
    },
    color: {
      domain: ["Proficient"],
      range: ["rgba(3,129,179,1)"],
      legend: false
    },

    marks: [
      // Stacked bars drawn as rects
      Plot.barX(segments, {
        x1: d => d.x1,
        x2: d => d.x2,
        y: d => d.entity,
        fill: d => d.color,
        stroke: "black",
        strokeWidth: 0.5
      }),

      // Count labels inside each bar segment
      Plot.text(segments, {
        x: d => d.x1 + 1,  // start of each segment
        y: d => d.entity,
        text: d => d.x2 - d.x1 > 2 ? d.label : "",  // hide label if segment too narrow
        fill: "black",
        textAnchor: "start"
      })
    ]
  });
}
```
:::